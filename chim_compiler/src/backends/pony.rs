...
impl PonyBackend {
    ...
    fn generate_type(&self, ty: &Type) -> String {
        match ty {
            ...
            Type::Ptr(inner) => format!("Pointer[{}]", self.generate_type(inner)),
            _ => "Any".to_string(),
        }
    }
...
impl CodegenBackend for PonyBackend {
    fn name(&self) -> &str {
        "Pony"
    }
    ...
    fn generate(&self, module: &Module) -> Result<String, Box<dyn Error>> {
        ...
        output.push_str("actor Main\n");
        output.push_str("  new create(env: Env) =>\n");
