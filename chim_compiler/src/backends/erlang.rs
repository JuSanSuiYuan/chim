use crate::backend::CodegenBackend;
use crate::ir::{Module, Function, Instruction, Type};
use std::error::Error;

pub struct ErlangBackend;

impl ErlangBackend {
    pub fn new() -> Self {
        Self
    }
    
    fn generate_type(&self, ty: &Type) -> String {
        match ty {
            Type::Void => "ok".to_string(),
            Type::Int32 | Type::Int64 => "integer()".to_string(),
            Type::Float32 | Type::Float64 => "float()".to_string(),
            Type::Bool => "boolean()".to_string(),
            Type::String => "string()".to_string(),
            Type::Ptr(_) => "term()".to_string(),
            _ => "any()".to_string(),
        }
    }
    
    fn generate_function(&self, func: &Function) -> String {
        let mut output = String::new();
        
        // 函数签名和类型规范
        output.push_str(&format!("-spec {}(", func.name));
        
        // 参数类型
        let param_types: Vec<String> = func.params.iter()
            .map(|(_, ty)| self.generate_type(ty))
            .collect();
        output.push_str(&param_types.join(", "));
        
        let ret_type = self.generate_type(&func.return_type);
        output.push_str(&format!(") -> {}.\n", ret_type));
        
        // 函数定义
        output.push_str(&format!("{}(", func.name));
        
        // 参数名
        let param_names: Vec<String> = func.params.iter()
            .map(|(name, _)| name.clone())
            .collect();
        output.push_str(&param_names.join(", "));
        
        output.push_str(") ->\n");
        
        // 函数体
        for inst in &func.body {
            output.push_str(&self.generate_instruction(inst));
        }
        
        output.push_str(".\n\n");
        output
    }
    
    fn generate_instruction(&self, inst: &Instruction) -> String {
        match inst {
            Instruction::Alloca { dest, .. } => {
                format!("    {} = undefined,\n", dest)
            },
            Instruction::Add { dest, left, right } => {
                format!("    {} = {} + {},\n", dest, left, right)
            },
            Instruction::Sub { dest, left, right } => {
                format!("    {} = {} - {},\n", dest, left, right)
            },
            Instruction::Mul { dest, left, right } => {
                format!("    {} = {} * {},\n", dest, left, right)
            },
            Instruction::Div { dest, left, right } => {
                format!("    {} = {} / {},\n", dest, left, right)
            },
            Instruction::Mod { dest, left, right } => {
                format!("    {} = {} rem {},\n", dest, left, right)
            },
            Instruction::Eq { dest, left, right } => {
                format!("    {} = {} =:= {},\n", dest, left, right)
            },
            Instruction::Ne { dest, left, right } => {
                format!("    {} = {} =/= {},\n", dest, left, right)
            },
            Instruction::Lt { dest, left, right } => {
                format!("    {} = {} < {},\n", dest, left, right)
            },
            Instruction::Le { dest, left, right } => {
                format!("    {} = {} =< {},\n", dest, left, right)
            },
            Instruction::Gt { dest, left, right } => {
                format!("    {} = {} > {},\n", dest, left, right)
            },
            Instruction::Ge { dest, left, right } => {
                format!("    {} = {} >= {},\n", dest, left, right)
            },
            Instruction::And { dest, left, right } => {
                format!("    {} = {} andalso {},\n", dest, left, right)
            },
            Instruction::Or { dest, left, right } => {
                format!("    {} = {} orelse {},\n", dest, left, right)
            },
            Instruction::Not { dest, src } => {
                format!("    {} = not {},\n", dest, src)
            },
            Instruction::Store { dest, src } => {
                format!("    {} = {},\n", dest, src)
            },
            Instruction::Load { dest, src } => {
                format!("    {} = {},\n", dest, src)
            },
            Instruction::Return(Some(value)) => {
                format!("    {}\n", value)
            },
            Instruction::Return(None) => {
                "    ok\n".to_string()
            },
            Instruction::ReturnInPlace(value) => {
                format!("    {} %% RVO优化\n", value)
            },
            Instruction::Call { dest, func, args } => {
                let args_str = args.join(", ");
                if let Some(d) = dest {
                    format!("    {} = {}({}),\n", d, func, args_str)
                } else {
                    format!("    {}({}),\n", func, args_str)
                }
            },
            _ => format!("    %% Unsupported: {:?}\n", inst),
        }
    }
}

impl CodegenBackend for ErlangBackend {
    fn name(&self) -> &str {
        "Erlang"
    }
    
    fn generate(&self, module: &Module) -> Result<String, Box<dyn Error>> {
        let mut output = String::new();
        
        // 模块声明
        output.push_str("-module(generated).\n");
        output.push_str("-compile(export_all).\n\n");
        
        // 导出所有函数
        output.push_str("-export([");
        let exports: Vec<String> = module.functions.iter()
            .map(|f| format!("{}/{}", f.name, f.params.len()))
            .collect();
        output.push_str(&exports.join(", "));
        output.push_str("]).\n\n");
        
        // 生成函数
        for func in &module.functions {
            output.push_str(&self.generate_function(func));
        }
        
        Ok(output)
    }
    
    fn file_extension(&self) -> &str {
        "erl"
    }
}
